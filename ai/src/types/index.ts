...
/**
 * Fixed small set used to sequence a daily routine (see agents/recommender.ts).
 * Classified once, offline, at catalog-ingestion time (data/pipeline/08-classify-categories.ts)
 * — not a live per-request LLM call. Undefined means the classifier couldn't
 * confidently place the product; it's left out of routine sequencing rather
 * than guessed.
 */
export type ProductCategory = 'cleanser' | 'treatment' | 'moisturizer' | 'spf' | 'exfoliant';
...
export interface Product {
  ...
  embedding?: number[];
  cachedAt?: Date;
  /** Routine-sequencing category, precomputed at ingestion time — see ProductCategory. */
  category?: ProductCategory;
}

/**
 * AM/PM sequencing of the final recommendations, generated by the same LLM
 * call as the recommendation explanations (see agents/recommender.ts). Values
 * are guidance text, not a safety verdict — see interactionWarnings note below.
 */
export interface Routine {
  am: string[];
  pm: string[];
  /**
   * LLM-generated guidance (e.g. "don't use retinol and AHA/BHA the same
   * night") — not a deterministic safety check. Distinct from safetyNotes,
   * which comes from the actual safety-checker verdict.
   */
