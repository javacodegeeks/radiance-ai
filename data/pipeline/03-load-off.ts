...
const LOCAL_FILE = path.join(CACHE_DIR, 'openfoodfacts-mongodbdump.gz');
...
function fetchChecksum(url: string): Promise<string | null> {
  return new Promise(resolve => {
    ...
    const req = client.get(url, res => {
      ...
      res.on('end', () => {
        const line = body.split(/\r?\n/).find(entry => entry.includes('openfoodfacts-mongodbdump.gz'));
...
export async function loadOFF(): Promise<void> {
  ...
  if (fs.existsSync(LOCAL_FILE) && fs.existsSync(CHECKSUM_FILE)) {
    ...
    console.log('  No cached dump — downloading (~14 GB)...');
    await downloadFile(DUMP_URL, LOCAL_FILE);
    if (remoteChecksum) fs.writeFileSync(CHECKSUM_FILE, remoteChecksum);
  }
  ...
  console.log(`  Restoring dump to MongoDB at ${host}:${port} (~80 GB)...`);
  execSync(
    `mongorestore --host ${host}:${port} --username ${user} --password ${password} --authenticationDatabase admin --gzip --archive="${LOCAL_FILE}" --nsFrom "off.*" --nsTo "${dbName}.*" --drop`,
    { stdio: 'inherit' },
  );
